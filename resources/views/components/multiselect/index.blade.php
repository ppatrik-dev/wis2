    ...
    <ul class="overflow-y-auto text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuhButton">
        @foreach ($options as $option)
            <li>
                ...
                <input id="checkbox-item-{{ $option->name }}" type="checkbox" value="" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                <label for="checkbox-item-{{ $option->name }}" class="w-full ms-2 text-sm font-medium text-gray-900 rounded-sm dark:text-gray-300">{{ ucfirst($option->name) }}</label>
                </div>
            </li>
        @endforeach
    </ul>
