<x-term::layouts.master>
    <x-header headline="Edit Room">
        <x-slot:actions>
            <x-button type="submit" form="roomDetailsForm" rounded="rounded-s-lg" variant="primary">
            ...
            <x-button href="{{ route('room.show', $room->id) }}" variant="danger">
    ...
    <x-term::profile :room="$room">
        <form id="roomDetailsForm" action="{{ route('room.update', $room->id) }}" method="POST"
            ...
            <x-input label="Capacity" name="capacity" value="{{ $room->capacity }}" type="number"></x-input>
            <x-input label="Location" name="location" value="{{ $room->location }}" class="col-span-3"></x-input>
        </form>
    </x-term::profile>
</x-term::layouts.master>