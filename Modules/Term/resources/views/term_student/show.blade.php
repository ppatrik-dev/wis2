<x-term::layouts.master>
    <x-header headline="Student Term Details">
        <x-slot:actions>
            <x-button href="{{ route('term.student.edit', [$term, $student->id]) }}" rounded="rounded-s-lg" variant="primary">
            ...
            <x-button form="termStudentForm" type="submit" variant="danger">
    ...
    <x-term::profile :student="$student">
        <form id="termStudentForm" action="{{ route('term.student.destroy', [$term, $student->id]) }}" method="POST"
            ...
            <x-input label="Score" name="score" type="number" value="{{ $student->score }}" :disabled="true" class="w-1/3 mx-auto"></x-input>
        </form>
    </x-term::profile>
