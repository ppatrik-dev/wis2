<x-term::layouts.master>
    <x-header headline="Edit Term">
        <x-slot:actions>
            ...
            <x-button href="{{ route('term.edit', $term->id) }}" variant="danger">
        ...
        <form id="termUpdateForm" action="{{ route('term.update', $term->id) }}" method="POST" class="grid w-full grid-cols-3 gap-6 py-10 mx-auto max-w-3/4">
            ...
            <x-input label="Description" name="description" value="{{ $term->description }}" input="textarea" rows="10" class="row-span-3"></x-input>
