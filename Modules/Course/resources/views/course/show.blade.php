<x-course::layouts.master>
    <x-header headline="Course Details">
        <x-slot:actions>
            @if(auth()->check() && auth()->user()->hasRole('admin') && !$course->is_approved)
                <form action="{{ route('course.approve', $course->id) }}" method="POST" class="inline">
                    @csrf
                    @method('PATCH')
                    <x-button type="submit" rounded="rounded-s-lg" variant="primary" class="relative z-10">
                        <svg class="w-3 h-3 me-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"/>
                        </svg>
                        Approve Course
                    </x-button>
                </form>
            ...
            @can('course.update', $course)
                <x-button href="{{ route('course.edit', $course->id) }}" rounded="{{ auth()->check() && auth()->user()->hasRole('admin') && !$course->is_approved ? '' : 'rounded-s-lg' }}" variant="primary">
                    <svg class="w-3 h-3 me-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
