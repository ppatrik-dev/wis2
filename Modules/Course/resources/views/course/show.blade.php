<x-course::layouts.master>
    <x-header headline="Course Details">
        <x-slot:actions>
            @can('course.update', $course)
                <x-button href="{{ route('course.edit', $course->id) }}"
    ...
    <x-course::profile :course="$course">
        <form id="courseDeleteForm" action="{{ route('course.destroy', $course->id) }}" method="POST"
            ...
            <x-input label="Guarantor" name="guarantor"
                value="{{ $course->guarantor ? $course->guarantor->first_name . ' ' . $course->guarantor->last_name : 'Not assigned' }}"
                :disabled="true"></x-input>
            <x-input label="Auto Enroll" name="auto_enroll_confirm"
                value="{{ $course->auto_enroll_confirm ? 'Yes' : 'No' }}" :disabled="true"></x-input>
            <x-input label="Status" name="is_approved" value="{{ $course->is_approved ? 'Approved' : 'Pending' }}"
                :disabled="true"></x-input>
            <x-input label="Description" name="description" value="{{ $course->description }}" input="textarea"
                :disabled="true" class="col-span-3"></x-input>
        </form>
    </x-course::profile>
</x-course::layouts.master>