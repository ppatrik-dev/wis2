            ...
            @if(isset($courses))
                @foreach($courses as $course)
                    <tr
                        ...
                        <td class="px-6 py-4">
                            <span
                                class="px-2 py-1 text-xs font-medium rounded-full {{ $course->is_approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                        ...
                        <td class="px-6 py-3">
                            @auth
                                @php
                                    $isEnrolled = isset($course->students) && auth()->check() && $course->students->contains('id', auth()->id());
                                @endphp

                                @if($isEnrolled)
                                    <form method="POST" action="{{ route('course.student.unregister', $course->id) }}"
                                        class="inline-flex items-center">
                                        @csrf
                                        <button type="submit"
                                            class="px-2 py-1 text-xs font-medium text-white bg-red-600 rounded hover:bg-red-700">Unregister</button>
                                    </form>
                                @else
                                    <form method="POST" action="{{ route('course.student.register', $course->id) }}"
                                        class="inline-flex items-center">
                                        @csrf
