<x-course::layouts.master>
    <x-header headline="Lecturer Details">
        <x-slot:actions>
            <x-button href="{{ route('course.lecturer.edit', [$courseId, $lecturerId]) }}" rounded="rounded-s-lg" variant="primary">
    ...
    <x-course::profile :courseLecturer="$courseLecturer">
        <form id="lecturerDeleteForm" action="{{ route('course.lecturer.destroy', [$courseId, $lecturerId]) }}" method="POST" class="w-full max-w-3/4 mx-auto py-10 grid grid-cols-3 gap-6"
            ...
            <x-input label="Assigned At" name="created_at" value="{{ $courseLecturer->created_at ? $courseLecturer->created_at->format('Y-m-d H:i') : 'N/A' }}" :disabled="true"></x-input>
        </form>
    </x-course::profile>
</x-course::layouts.master>
